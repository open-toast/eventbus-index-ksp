...
class BuilderGenerator(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        val methods = subscribeMethods(resolver).map {
            parseSubscribeMethod(it)
        }.partition()

        for (invalid in methods.second) {
            logger.warn(invalid.why)
        }

        if (methods.first.isNotEmpty()) {
            codeGenerator.write(options.getValue(OPTION_EVENT_BUS_INDEX), methods.first)
        }
